...
export function EditFieldMappings({ collection, availableModels = [] }: { collection: any, availableModels?: string[] }) {
  ...
  useEffect(() => {
    setFields([...collection.fieldDefinitions].sort((a: any, b: any) => (a.uiOrder || 0) - (b.uiOrder || 0)));
  ...
  const handleDragOver = (event: any) => {
    ...
    setFields(prev => {
      ...
      if (isCurrentlyAdmin !== shouldBeAdmin) {
        ...
        const mutableActiveIdx = mutableFields.findIndex(f => f.id === active.id);
        if (mutableActiveIdx === -1) return prev;
        
        const mutableItem = mutableFields[mutableActiveIdx];
        mutableFields.splice(mutableActiveIdx, 1);
        
        const movedItem = { ...mutableItem, isAdministrative: shouldBeAdmin };
        
        if (shouldBeAdmin) {
          mutableFields.push(movedItem);
        } else {
          const firstAdminIndex = mutableFields.findIndex(f => f.isAdministrative);
          if (firstAdminIndex === -1) {
            mutableFields.push(movedItem);
          } else {
            mutableFields.splice(firstAdminIndex, 0, movedItem);
          }
        }
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (active.id !== over.id) {
      setFields(prev => {
        const item = prev.find(f => f.id === active.id);
        if (!item) return prev;
        
        const isCurrentAdmin = item.isAdministrative;
        
        let desc = prev.filter(f => !f.isAdministrative);
        let admin = prev.filter(f => f.isAdministrative);
        
        let targetList = isCurrentAdmin ? admin : desc;
        ...
        return isCurrentAdmin ? [...desc, ...targetList] : [...targetList, ...admin];
      });
    }
  };
